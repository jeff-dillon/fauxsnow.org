    ...
    if load_data:
        ...
        for forecast in foreacsts:
            ...
            for i in range(3,10):
                ...
                record[f'fp{i-2}_date'] = forecast['daily']['time'][i]
                record[f'fp{i-2}_day_short'] = weather.get_short_day(forecast['daily']['time'][i])
                record[f'fp{i-2}_day_long'] = weather.get_long_day(forecast['daily']['time'][i])
                record[f'fp{i-2}_max_temp'] = forecast['daily']['temperature_2m_max'][i]
                record[f'fp{i-2}_min_temp'] = forecast['daily']['temperature_2m_min'][i]
                record[f'fp{i-2}_conditions'] = weather.get_conditions(forecast['daily']['weathercode'][i])
                record[f'fp{i-2}_fs_conditions'] = weather.get_fs_conditions(float(avg_dewpoint), 
